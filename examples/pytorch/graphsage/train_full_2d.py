...
def twod_partition(rank, size, inputs, adj_matrix, normalization): #Data? features? classes? device?
    ...
    n_per_proc = math.ceil(float(node_count) / proc_row) #in intel, this is ceiling/float, in CAGNET it is not
    ...
    with torch.no_grad():
        ...
        for i in range(len(am_partitions)):
            ...
            am_partitions[i] = torch.sparse_coo_tensor(am_partitions[i], jh
...
def main(args):
    ...
    features_loc, g_loc, ampbyp = twod_partition(rank, size, args.replication, features, edges, 
                                                        args.normalization)
    # print("------------------->",ampbyp)
