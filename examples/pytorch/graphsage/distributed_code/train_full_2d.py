...
from sageconv_2d import SAGEConvAgg, SAGEConvMLP
# import torch_ccl
...
class GraphSAGE(nn.Module):
  def __init__(self, in_feats, n_hidden, n_classes, n_layers, activation, dropout, aggregator_type, rank, size, replication, group, row_groups, col_groups, node_count):
    ...
    # input layer
    self.agg_layers.append(SAGEConvAgg(in_feats, n_hidden, aggregator_type))
    # hidden layers
    for i in range(n_layers - 1):
        self.agg_layers.append(SAGEConvAgg(n_hidden, n_hidden, aggregator_type))
    # output layer
    self.agg_layers.append(SAGEConvAgg(n_hidden, n_classes, aggregator_type)) # activation None
  ...
  def forward(self, graph, inputs, ampbyp):
    ...
    for l, agg_layer in enumerate(self.agg_layers):
      h = agg_layer(self, graph, ampbyp, h)
      #h = mlp_layer(graph, z, z)
    return h

# Normalize all elements according to KW's normalization rule
def scale_elements(adj_matrix, adj_part, node_count, row_vtx, col_vtx, normalization): #Do we need a normalization input?
...
def split_coo(adj_matrix, node_count, n_per_proc, dim, size):
    ...
    vtx_indices = vtx_indices[:proc_row] #only change from 1d to 2d
...
def twod_partition(rank, size, inputs, adj_matrix, normalization): #Data? features? classes? device?
        ...
        for i in range(len(am_pbyp)):
            if i == proc_row - 1:
                ...
                am_pbyp[i] = scale_elements(adj_matrix, am_pbyp[i], node_count, vtx_indices[i], 
                                                    vtx_indices[rank_col], normalization)
            else:
                am_pbyp[i] = torch.sparse_coo_tensor(am_pbyp[i], torch.ones(am_pbyp[i].size(1)), 
...
def main(args):
    ...
    features_loc, g_loc, ampbyp, node_count = twod_partition(rank, size, features, edges, 
                                                        args.normalization)
    # print("------------------->",ampbyp)
    ...
    for i in range(len(ampbyp)):
        ...
        ampbyp_dgl.append(dgl.create_block((ampbyp[i]._indices()[1], 
                                            ampbyp[i]._indices()[0]),
