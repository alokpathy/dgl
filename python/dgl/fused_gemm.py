...
class FusedGEMMBlockSpMM(torch.autograd.Function):
    ...
    def forward(ctx, a_mats, b_mats, a_mats_rows):
        ...
        torch.cuda.nvtx.range_push("nvtx-constructarrs")
        da_mats_rows = a_mats_rows.cuda()
        padding_arr = torch.cuda.IntTensor(a_mats_rows.size(0))
        torch.cuda.nvtx.range_pop()
        torch.cuda.nvtx.range_push("nvtx-todglwrite")
        arg_padding_arr = to_dgl_nd_for_write(padding_arr)
        ...
        _CAPI_DGLKernelPadBlockSpMM(arg_a_pad, arg_a_mat, arg_b_mats, arg_a_mats_rows, \
                                        arg_da_mats_rows, arg_padding_arr, \
                                        num_edges, num_edges + padding, a_pad.size(1), b_mats.size(1), \
